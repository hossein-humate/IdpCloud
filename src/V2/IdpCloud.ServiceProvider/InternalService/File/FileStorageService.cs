...
namespace IdpCloud.ServiceProvider.InternalService.File
{
    public class FileStorageService : IFileStorageService
    {
        ...
        public async Task<string> StoreInSpecificIdPathAsync(Guid specificId, string fileName, byte[] fileContent,
            string newFolder = default, CancellationToken cancellationToken = default)
        {
            try
            {
                var path = newFolder == default ? Path.Combine(Environment.CurrentDirectory + _configuration["GlobalParameter:StoragePath"],
                    specificId.ToString()) : Path.Combine(Environment.CurrentDirectory + _configuration["GlobalParameter:StoragePath"],
                    newFolder, specificId.ToString());
                if (!Directory.Exists(path)) Directory.CreateDirectory(path);
                path = $"{path}{Path.DirectorySeparatorChar}{fileName}";
                await using var fileStream = System.IO.File.Create(path);
